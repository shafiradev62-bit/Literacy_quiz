          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-4 exam-scrollbar">
              ...
              <div className="space-y-5">
                ...
                <div className="text-[13px] leading-[1.8] text-foreground/80 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["/images/units/10/img1.jpeg","/images/units/10/img2.jpeg"].map((src) => (
                      <div key={src} className="rounded-xl overflow-hidden border border-border/40 aspect-[4/3] bg-muted/20">
                        <img src={src} alt="Tahu gejrot" className="w-full h-full object-cover" />
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  {Object.keys(q1Answers).length >= 3 && (
                    <p className={`text-[11px] font-semibold ${isQ1Correct() ? "text-emerald-600" : "text-rose-600"}`}>
                      {isQ1Correct()
                        ? (isId ? "Sudah tepat. Bahan masuk, makanan disiapkan, limbah diolah." : "Placed correctly. Ingredients enter, food is prepared, wastewater is treated.")
                        : (isId ? "Coba lagi. Cek pasangan input–proses–pengolahan." : "Try again. Recheck input–process–treatment mapping.")}
                    </p>
                  )}
            ...
            {currentStep === 2 && (
              <div className="space-y-5">
                ...
                <div className="bg-muted/30 border border-border/60 rounded-lg p-4 text-[12px] text-foreground/70 space-y-2">
                  <p className="font-bold text-[10px] uppercase tracking-widest text-muted-foreground mb-1">{isId ? "Limbah & BOD/COD" : "Wastewater & BOD/COD"}</p>
                  <p>{isId
                    ? "Limbah cair dari produksi tahu dapat mengandung bahan organik tinggi dan meningkatkan BOD serta COD jika dibuang tanpa pengolahan."
                    : "Wastewater from tofu production can contain high organic matter and can increase BOD and COD if discharged without treatment."}</p>
                  <table className="w-full text-[11px] border-collapse mt-2">
                    <thead><tr className="bg-muted/50">
                      ...
                      <th className="border border-border/40 px-2 py-1 text-left">{isId ? "Dampak pada air" : "Possible effect on water"}</th>
                    </tr></thead>
                    <tbody>
                      <tr><td className="border border-border/40 px-2 py-1">{isId ? "Limbah tahu tanpa pengolahan" : "Untreated tofu wastewater"}</td><td className="border border-border/40 px-2 py-1">{isId ? "BOD/COD lebih tinggi dan kualitas air menurun" : "Higher BOD/COD and poorer water quality"}</td></tr>
                      <tr><td className="border border-border/40 px-2 py-1">{isId ? "Limbah diolah sebagian" : "Partially treated wastewater"}</td><td className="border border-border/40 px-2 py-1">{isId ? "Pencemaran berkurang, tetapi masih ada risiko" : "Lower pollution, but some risk remains"}</td></tr>
                      <tr><td className="border border-border/40 px-2 py-1">{isId ? "Limbah diolah penuh" : "Fully treated wastewater"}</td><td className="border border-border/40 px-2 py-1">{isId ? "Risiko pencemaran paling rendah" : "Lowest pollution risk"}</td></tr>
                    </tbody>
                  </table>
                </div>
                <p className="text-[13px] font-medium text-foreground/90 leading-relaxed whitespace-pre-line">{isId
                  ? "Perhatikan informasi dan hasil simulasi. Para peneliti menemukan bahwa limbah cair dari produksi tahu dapat mengandung bahan organik tinggi dan dapat meningkatkan BOD dan COD jika dibuang tanpa pengolahan. Tindakan manakah yang paling efektif untuk mengurangi masalah ini dalam sistem pangan tahu gejrot?"
                  : "Refer to the information and the simulation results. Researchers notice that wastewater from tofu production can contain high organic matter and can increase BOD and COD if it is discharged without treatment. Which action would best reduce this problem in a tahu gejrot food system?"}</p>
                <div className="grid gap-3">
                  {[
                    { val: "A", id: "Menggunakan porsi tahu yang lebih besar", en: "Using a larger serving size of tofu", bg: "bg-rose-50/50" },
                    { val: "B", id: "Mengganti tahu dengan lebih banyak makanan berbasis hewani", en: "Replacing tofu with more animal-based food", bg: "bg-blue-50/50" },
                    { val: "C", id: "Mengolah limbah cair tahu sebelum dibuang", en: "Treating tofu wastewater before it is released", bg: "bg-emerald-50/50" },
                    { val: "D", id: "Menambahkan lebih banyak gula pada kuah", en: "Adding more sugar to the sauce", bg: "bg-slate-50/40" },
                  ].map((opt, idx) => (
                    <label 
                      key={opt.val} 
            ...
            {currentStep === 5 && (
              <div className="space-y-5">
                ...
                <p className="text-[13px] font-medium text-foreground/90 leading-relaxed whitespace-pre-line">{isId
                  ? "Tahu gejrot merupakan makanan tradisional Cirebon sekaligus bagian dari perekonomian lokal. Jelaskan mengapa penggunaan bahan lokal, protein nabati, porsi yang wajar, dan pengolahan limbah yang tepat dapat mendukung sistem pangan lokal yang lebih berkelanjutan."
                  : "Tahu gejrot is both a traditional Cirebon food and part of the local economy. Explain why using local ingredients, plant-based protein, reasonable portions, and proper wastewater treatment can support a more sustainable local food system."}</p>
                <WritingGuideBtn
                  open={showWritingGuide5}
                  setOpen={setShowWritingGuide5}
                  text={isId
                    ? "Protein nabati dapat mengurangi kebutuhan sumber daya. Bahan lokal mendukung rantai pasok pendek dan ketahanan pangan. Porsi wajar mengurangi penggunaan bahan dan limbah makanan. Pengolahan limbah mencegah pencemaran perairan dan melindungi ekosistem air."
                    : "Plant-based protein can reduce resource demand. Local ingredients support short supply chains and food resilience. Reasonable portions can reduce material use and food waste. Wastewater treatment helps prevent pollution of waterways and protects aquatic ecosystems."} />
